package com.murilloskills.forge112.config;
...
public final class Forge112ConfigLoader {
    private Forge112ConfigLoader() {
    }

    public static SkillProgressionConfig load(Path configPath) {
        SkillProgressionConfig defaults = SkillProgressionConfig.DEFAULT;
        if (!Files.isRegularFile(configPath)) {
            LOG.info("[MurilloSkills][1.12.2] Config not found at {}; using built-in progression defaults.", configPath);
            return defaults;
        }
        try {
            ...
            if (parsed == null || !parsed.isJsonObject()) {
                LOG.warn("[MurilloSkills][1.12.2] Config at {} is not a JSON object; using built-in progression defaults.", configPath);
                return defaults;
            }
            JsonObject root = parsed.getAsJsonObject();
            JsonObject general = object(root, "general");
            JsonObject xp = object(root, "xp");
            JsonObject prestige = object(root, "prestige");
            SkillProgressionConfig loaded = new SkillProgressionConfig(
                    intValue(general, "maxLevel", defaults.getMaxLevel()),
                    intValue(general, "maxSelectedSkills", defaults.getMaxSelectedSkills()),
                    intValue(xp, "base", defaults.getXpBase()),
                    intValue(xp, "multiplier", defaults.getXpMultiplier()),
                    intValue(xp, "exponent", defaults.getXpExponent()),
                    floatValue(prestige, "xpBonus", defaults.getPrestigeXpBonus()),
                    floatValue(prestige, "passiveBonus", defaults.getPrestigePassiveBonus()),
                    floatValue(prestige, "cooldownReductionPerLevel", defaults.getPrestigeCooldownReductionPerLevel()),
                    floatValue(prestige, "maxCooldownReduction", defaults.getMaxPrestigeCooldownReduction()),
                    intValue(prestige, "maxPrestigeLevel", defaults.getMaxPrestigeLevel()));
            LOG.info("[MurilloSkills][1.12.2] Loaded progression config from {}", configPath);
            return loaded;
        } catch (Exception e) {
            LOG.warn("[MurilloSkills][1.12.2] Failed to load progression config from {}; using built-in progression defaults.", configPath, e);
            return defaults;
        }
    }

    public static JsonObject object(JsonObject root, String name) {
    ...
    public static float floatValue(JsonObject object, String name, float fallback) {
        JsonElement element = object.get(name);
        if (element == null || element.isJsonNull()) {
            return fallback;
        }
