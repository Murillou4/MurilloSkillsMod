...
public final class Forge112ClientHudOverlay {
    @SubscribeEvent
    public static void onOverlay(RenderGameOverlayEvent.Text event) {
        if (!Boolean.getBoolean("murilloskills.debugOverlay")) {
            return;
        }
        ...
        right.add("[MurilloSkills] " + data.getSelectedSkills() + " P=" + data.getActiveParagonSkill());
        SkillType active = activeAbilitySkill(data);
        if (active != null) {
            SkillStatsCore stats = data.getSkill(active);
            right.add("[Ability] " + skillName(active) + " L" + stats.getLevel() + " P" + stats.getPrestige());
        }
    }
}
