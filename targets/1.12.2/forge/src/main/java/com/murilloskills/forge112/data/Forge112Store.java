package com.murilloskills.forge112.data;
...
public final class Forge112Store {
    ...
    public PlayerSkillDataCore load(UUID uuid) {
        PlayerSkillDataCore cached = cache.get(uuid);
        if (cached != null) {
            return cached;
        }
        Path path = path(uuid);
        PlayerSkillDataCore data;
        try {
            data = Files.exists(path) ? PlayerSkillJsonCodec.read(path) : new PlayerSkillDataCore();
        } catch (IOException error) {
            LOG.warn("[MurilloSkills][1.12.2] Failed to read {}: {}", path, error.toString());
            data = new PlayerSkillDataCore();
        }
        cache.put(uuid, data);
        return data;
    }
    ...
    private Path path(UUID uuid) {
        Path base = root == null ? new java.io.File("murilloskills-1.12.2").toPath() : root;
        return base.resolve("players").resolve(uuid.toString() + ".json");
    }
}
