...
extends DataFix {
    ...
    private static void fixStack(StackData data, Dynamic<?> dynamic3) {
        ...
        data.applyFixer("BlockEntityTag", false, blockEntityTagDynamic -> {
            ...
            Dynamic dynamic = (blockEntityTagDynamic = ItemStackComponentizationFix.fixBlockEntityData(data, blockEntityTagDynamic, string)).remove("id");
    ...
    private static Dynamic<?> fixDisplay(StackData data, Dynamic<?> dynamic2, int hideFlags) {
        Optional optional2;
        boolean bl;
        dynamic2.get("Name").result().filter(TextFixes::method_74084).ifPresent(dynamic -> data.setComponent("minecraft:custom_name", (Dynamic<?>)dynamic));
        OptionalDynamic optionalDynamic = dynamic2.get("Lore");
        if (optionalDynamic.result().isPresent()) {
            data.setComponent("minecraft:lore", dynamic2.createList(dynamic2.get("Lore").asStream().filter(TextFixes::method_74084)));
        }
        ...
        if (optional.isPresent() || bl) {
            ...
            data.setComponent("minecraft:dyed_color", dynamic22);
        }
        if ((optional2 = dynamic2.get("LocName").asString().result()).isPresent()) {
            data.setComponent("minecraft:item_name", TextFixes.translate(dynamic2.getOps(), (String)optional2.get()));
        }
        if (data.itemEquals("minecraft:filled_map")) {
            data.setComponent("minecraft:map_color", dynamic2.get("MapColor"));
    ...
    private static <T> Dynamic<T> fixBlockEntityData(StackData data, Dynamic<T> dynamic, String blockEntityId) {
        ...
        return switch (blockEntityId) {
            ...
            case "minecraft:banner" -> {
                ...
                Optional optional2 = dynamic.get("Base").asNumber().result();
                if (optional2.isPresent()) {
    ...
    private static void fixEnchantments(StackData data, Dynamic<?> dynamic, String nbtKey, String componentId, boolean hideInTooltip) {
        ...
        List<Pair> list = optionalDynamic.asList(Function.identity()).stream().flatMap(enchantmentsDynamic -> ItemStackComponentizationFix.getEnchantmentAndLevelPair(enchantmentsDynamic).stream()).filter(pair -> (Integer)pair.getSecond() > 0).toList();
        if (!list.isEmpty() || hideInTooltip) {
            Dynamic dynamic2 = dynamic.emptyMap();
            Dynamic dynamic3 = dynamic.emptyMap();
            for (Pair pair2 : list) {
                dynamic3 = dynamic3.set((String)pair2.getFirst(), dynamic.createInt(((Integer)pair2.getSecond()).intValue()));
            }
    ...
    private static Optional<Pair<String, Integer>> getEnchantmentAndLevelPair(Dynamic<?> dynamic) {
        return dynamic.get("id").asString().apply2stable((enchantmentId, level) -> Pair.of((Object)enchantmentId, (Object)MathHelper.clamp(level.intValue(), 0, 255)), dynamic.get("lvl").asNumber()).result();
    }
    ...
    private static Dynamic<?> createBlockPredicateListDynamic(Dynamic<?> dynamic, String listAsString) {
        ...
        if (i != -1 && l != -1) {
            Dynamic dynamic3 = dynamic.emptyMap();
            Iterable iterable = COMMA_SPLITTER.split((CharSequence)listAsString.substring(i + 1, l));
    ...
    private static Dynamic<?> fixAttributeModifier(Dynamic<?> dynamic) {
        ...
        dynamic2 = Dynamic.copyField(dynamic, (String)"AttributeName", (Dynamic)dynamic2, (String)"type");
        dynamic2 = Dynamic.copyField(dynamic, (String)"Slot", (Dynamic)dynamic2, (String)"slot");
        dynamic2 = Dynamic.copyField(dynamic, (String)"UUID", (Dynamic)dynamic2, (String)"uuid");
        dynamic2 = Dynamic.copyField(dynamic, (String)"Name", (Dynamic)dynamic2, (String)"name");
        dynamic2 = Dynamic.copyField(dynamic, (String)"Amount", (Dynamic)dynamic2, (String)"amount");
        dynamic2 = Dynamic.copyAndFixField(dynamic, (String)"Operation", (Dynamic)dynamic2, (String)"operation", operationDynamic -> operationDynamic.createString(switch (operationDynamic.asInt(0)) {
            default -> "add_value";
    ...
    private static void fixPotionContents(StackData data, Dynamic<?> dynamic) {
        ...
        if (!(dynamic2 = data.moveToComponent("custom_potion_effects", dynamic2, "custom_effects")).equals((Object)dynamic.emptyMap())) {
    ...
    private static Dynamic<?> fixBookPages(StackData data, Dynamic<?> dynamic) {
        List list = data.getAndRemove("pages").asList(pagesDynamic -> pagesDynamic.asString(""));
        Map map = data.getAndRemove("filtered_pages").asMap(filteredPagesKeyDynamic -> filteredPagesKeyDynamic.asString("0"), filteredPagesValueDynamic -> filteredPagesValueDynamic.asString(""));
        if (list.isEmpty()) {
            return null;
        }
    ...
    public static Dynamic<?> createProfileDynamic(Dynamic<?> dynamic) {
        ...
        if (optional.isPresent()) {
            if (ItemStackComponentizationFix.isValidUsername((String)optional.get())) {
