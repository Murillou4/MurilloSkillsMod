1. FARMER (AGRICULTOR)

Tema: Plantações, crescimento, alimentos naturais.

Fontes de XP:

| Ação | XP |
| :--- | :---: |
| Colher cultivos maduros | Base |
| Plantar sementes | +25% do base |
| Compostar itens | +15% do base |
| Criar Bone Meal na Composteira | +10% do base |

Bônus por Nível:
- +0.5% de chance de colheita dupla por nível (máx 50% no nível 100).

Mecânica Bônus - Qualidade de Cultivo:
- Chance de colher itens "Dourados" (+50% valor de nutrição).
- Chance base: 0.15% por nível (máx 15% no nível 100).

Perks por Nível:

| Nível | Habilidade / Recompensa |
| :---: | :--- |
| 10 | **Green Thumb**: +5% de chance de colheita extra. Sementes têm 10% de chance de não serem consumidas ao plantar. |
| 25 | **Fertile Ground**: Plantações crescem 25% mais rápido. |
| 50 | **Nutrient Cycle**: Compostar itens dá 2x Bone Meal. Colheitas têm 5% de chance de dropar sementes extras. |
| 75 | **Abundant Harvest**: +15% de colheita extra permanente. 10% de chance de colher cultivos adjacentes (1 bloco de raio). |
| 100 | **Master Farmer** (Habilidade Ativa): Ao pressionar F, ativa **Harvest Moon** — colhe automaticamente todos os cultivos maduros em um raio de 8 blocos. Durante 10 segundos: 100% de chance de colheita tripla, cultivos colhidos são automaticamente replantados. Recarga: 2 minutos. |

---

2. FISHER (PESCADOR)

Tema: Pesca, água e domínio aquático.

Fontes de XP:
- Pescar (Tesouro > Peixe > Lixo).

Bônus por Nível:
- +0.5% de velocidade de pesca.
- +0.3% de chance de pescar um Bundle Épico (máx 30% no nível 100).

Conteúdo do Bundle Épico (1 item sorteado por peso):

| Chance | Item |
| :---: | :--- |
| 40% | Livro Encantado OP (Mending, Frost Walker, Depth Strider, Looting III, Fortune III, Efficiency V, Sharpness V ou Protection V) |
| 30% | 1-3x Bloco de Ouro |
| 15% | 1-2x Bloco de Diamante |
| 10% | Tridente (com 1 encantamento aleatório de nível alto) |
| 4% | 1x Bloco de Netherite |
| 1% | 1x Maçã Dourada Encantada |

Perks por Nível:

| Nível | Habilidade / Recompensa |
| :---: | :--- |
| 10 | -25% de tempo de espera entre fisgadas. |
| 25 | +10% de chance de pescar tesouros raros + 10% mais XP ao pescar. |
| 50 | Dolphin's Grace leve (velocidade aumentada na água). |
| 75 | Luck of the Sea passivo nível 1 (efeito invisível, mas ativo). |
| 100 | Master Fisher (Habilidade Ativa): Ao pressionar F, ativa Rain Dance — invoca chuva visual temporária por 60 segundos. Durante o efeito: +50% velocidade de pesca, +30% chance de tesouro, chance tripla de Bundle Épico, e Conduit Power enquanto na água. Recarga: 30 minutos. |

---

3. MINER (MINERADOR)

Tema: Cavernas, profundezas.

Fontes de XP:
- Quebrar pedras e minérios.

Bônus por Nível:
- +3% de velocidade de mineração (Haste).
- +3% de chance de Fortuna.

Perks por Nível:

| Nível | Habilidade / Recompensa |
| :---: | :--- |
| 15 | Visão noturna (Night Vision) leve em cavernas. |
| 30 | Ferramentas duram 15% mais. |
| 60 | Radar sonoro de minério (emite som ao se aproximar). |
| 100 | Master Miner (Habilidade Ativa): Ao pressionar F, ativa um pulso geológico. Revela minerais próximos em um raio de 6 a 10 blocos (efeito outline por 3 segundos). Recarga: 1 hora. |

---

4. BUILDER (CONSTRUTOR)

Tema: Construção, arquitetura, domínio espacial.

Fontes de XP:
- Colocar blocos de construção.
- Craftar blocos estruturais e decorativos.

Bônus por Nível:
- +0.05 blocos de alcance para colocar blocos por nível (máx +5 blocos no nível 100).

Perks por Nível:

| Nível | Habilidade / Recompensa |
| :---: | :--- |
| 10 | **Extended Reach**: +1 bloco de alcance ao construir. Ganho de mais espaço no inventário (linha extra). 
| 15 | **Efficient Crafting**: 20% de economia de materiais ao craftar blocos decorativos (escadas, lajes, cercas, etc.). |
| 25 | **Safe Landing**: 25% menos dano de queda. |
| 50 | **Scaffold Master**: Velocidade aumentada ao subir scaffolding (andaimes). 50% de economia de materiais ao craftar blocos estruturais. |
| 75 | **Master Reach**: +5 blocos de alcance ao construir (cumulativo com bônus por nível). |
| 100 | **Master Builder** (Habilidade Ativa): Ao pressionar F, ativa **Creative Brush** — por 20 segundos, você pode colocar blocos em área ao arrastar o cursor, como se estivesse "pintando o chão". Blocos preenchem a linha traçada automaticamente. Consome blocos do inventário. Recarga: 10 minutos. |

---

5. BLACKSMITH (FERREIRO)
Tema: Forja, resistência, armaduras impenetráveis.
Fontes de XP:
- Usar bigorna (Anvil).
- Encantar ferramentas e armaduras.
- Fundir minérios em fornalhas.
- Usar o rebolo (Grindstone).
- Reparar itens (via bigorna ou crafting).
Bônus por Nível:
- +2% de resistência física por nível (máx 200% no nível 100).

Perks por Nível:
| Nível | Habilidade / Recompensa |
| :---: | :--- |
| 10 | **Iron Skin**: +5% de resistência a dano físico passivo. |
| 25 | **Efficient Anvil**: 25% de desconto de XP no uso da bigorna. 10% de chance de não consumir material ao reparar. |
| 50 | **Forged Resilience**: +10% de resistência a dano de fogo e explosão. Armaduras ganham +1 nível de Protection efetivo (invisível, mas ativo). |
| 75 | **Thorns Master**: 20% de chance de refletir 25% do dano recebido ao atacante. Knockback recebido é reduzido em 50%. |
| 100 | **Master Smith** (Habilidade Ativa): Ao pressionar F, ativa **Titanium Aura** — por 15 segundos: imunidade a knockback, +30% de resistência a todo tipo de dano, regeneração lenta (0.5 coração/seg), e armaduras não perdem durabilidade. 25% de chance passiva de obter encantamentos acima do limite ao usar a bigorna (ex: Protection V, Unbreaking IV). Recarga: 1 hora. |

---

6. EXPLORER (EXPLORADOR)

Tema: Descoberta, terreno, sobrevivência em viagens longas.

Fontes de XP:

| Ação | XP |
| :--- | :---: |
| Descobrir novo bioma | 50 |
| Descobrir nova estrutura | 100 |
| Abrir baú de loot (primeira vez) | 30 |
| Completar um Mapa | 200 |
| Trocar com Wandering Trader | 20 por trade |

Bônus por Nível:
- +0.2% de velocidade de movimento por nível (máx +20% no nível 100).
- +1 de Sorte (Luck) a cada 20 níveis (melhora loot de baús gerados e pesca).
- Redução gradual do consumo de fome ao andar.

Perks por Nível:

| Nível | Habilidade / Recompensa |
| :---: | :--- |
| 10 | **Passo Leve (Step Assist)**: Sobe blocos automaticamente sem pular (como cavalo/cabra). |
| 20 | **Aquático (Aqua Affinity)**: Respira 50% mais tempo submerso. Minera em velocidade normal debaixo d'água. |
| 35 | **Olhos de Gato (Night Vision)**: Visão noturna permanente (pode ser desativada pressionando Z). |
| 65 | **Pés de Pena (Feather Feet)**: Reduz dano de queda em 40%. |
| 80 | **Caminhante do Nether**: Imune a dano de Magma Blocks. Velocidade normal na Soul Sand sem encantamento Soul Speed. |
| 100 | **Master Explorer** (Habilidade Passiva): **Sexto Sentido (Treasure Hunter)** — Todos os Baús e Spawners em um raio de 32 blocos ganham efeito de Glowing visível através de paredes. Nunca mais perca uma sala secreta! |

---

7. WARRIOR (GUERREIRO)

Fontes de XP:
- Receber dano e sobreviver.
- Causar dano corpo a corpo (Melee).
- Defender aldeias.
- Bloquear ataques com escudo.

Bônus por Nível:
- +0.05 de dano físico.
- +0.1 de Looting (Saque) passivo (funciona mesmo sem encantamento).

Perks por Nível:

| Nível | Habilidade / Recompensa |
| :---: | :--- |
| 10 | +1 coração de vida máxima. |
| 25 | +15% de resistência a dano. |
| 50 | +1 coração de vida máxima. |
| 75 | Roubo de vida (Lifesteal) leve de 15% ao atacar. |
| 100 | Master Warrior: +3 corações de vida máxima. |

---

8. ARCHER (ARQUEIRO)

Fontes de XP:
- Acertar flechas (Arco e Besta).
- Acertos de longa distância (Long range).

Bônus por Nível:
- +2% de dano à distância.

Perks por Nível:

| Nível | Habilidade / Recompensa |
| :---: | :--- |
| 10 | Aumento na velocidade do projétil (flechas voam mais rápido). |
| 25 | +5% de dano à distância. |
| 50 | Flechas ganham propriedade de perfuração (Piercing). |
| 75 | Disparo estável (redução da dispersão/spread da flecha). |
| 100 | Master Ranger: Flechas perfuram múltiplos alvos e perseguem o último inimigo que causou dano. Duração do efeito: 30 segundos. |