...
public class SkillsScreen extends Screen {
    ...
    private int cardActionButtonHeight() {
        return Math.max(14, Math.min(16, cardHeight / 3));
    }

    private int cardBottomButtonSize() {
        return Math.max(14, Math.min(16, cardHeight / 5));
    }

    private int cardBottomButtonY(int cardY, int buttonHeight) {
        return cardY + cardHeight - buttonHeight - 8;
    }
    ...
    private void calculateResponsiveLayout() {
        ...
        // Margem minima das bordas
        int marginX = selectionMode ? 16 : 20;
        this.headerHeight = selectionMode
                ? Math.max(30, Math.min(42, this.height / 7))
                : Math.max(35, this.height / 12);
        int topGap = selectionMode ? 6 : 10;
        int bottomReserve = selectionMode ? 34 : 50;
        ...
        // Selection mode keeps two columns so the first-choice screen stays readable.
        if (!selectionMode && availableWidth >= 700) {
            this.columns = 4;
        ...
        this.padding = selectionMode
                ...
                : Math.max(6, Math.min(12, availableWidth / 40));

        int totalPaddingX = (columns - 1) * padding;
        int totalPaddingY = (rows - 1) * padding;

        this.cardWidth = Math.min(selectionMode ? 190 : 180,
                Math.max(120, (availableWidth - totalPaddingX) / columns));

        int minCardHeight = selectionMode ? 46 : 62;
        int compactMinCardHeight = selectionMode ? 38 : 54;
        ...
        if (neededHeight > availableHeight) {
            this.cardHeight = Math.max(compactMinCardHeight, (availableHeight - totalPaddingY) / rows);
        }
            ...
            for (int i = 0; i < skills.length; i++) {
                ...
                if (isSelected) {
                    ...
                    if (skill == MurilloSkillsList.MINER) {
                        ...
                        int configBtnX = x + 4;
                        int configBtnY = cardBottomButtonY(y, configBtnSize);

                        ButtonWidget configBtn = ButtonWidget
                                .builder(Text.literal("F"), (button) -> {
                    ...
                    case BLACKSMITH -> {
                        ...
                        tooltip.add(
                                Text.translatable("murilloskills.passive.blacksmith.physical_resistance", resistance)
                                        .append(Text.literal(prestigeIndicator).formatted(Formatting.LIGHT_PURPLE))
                                        .formatted(Formatting.GOLD));
                        float furnaceSpeed = 1.0f
                                + (Math.min(level, SkillConfig.getMaxLevel()) / (float) SkillConfig.getMaxLevel())
                                        * (SkillConfig.getBlacksmithFurnaceSpeedEffectiveMaxMultiplier() - 1.0f);
