...
/**
 * Item picker for the Storage routing whitelist. Mirror of TrashItemPickerScreen, but
 * adding/removing toggles items into the whitelist that controls which ultmine drops are
 * pushed to the bound Tom's Storage terminal.
 */
public class StorageWhitelistPickerScreen extends Screen {
    ...
    public StorageWhitelistPickerScreen(Screen parent) {
        super(Text.translatable("murilloskills.storage_picker.title"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        searchField.setPlaceholder(Text.translatable("murilloskills.storage_picker.search_placeholder")
        ...
        ButtonWidget pickHeldBtn = ButtonWidget.builder(
                Text.translatable("murilloskills.storage_picker.pick_held").formatted(Formatting.AQUA),
                (b) -> addHeldItem())
        ...
        ButtonWidget doneBtn = ButtonWidget.builder(
                Text.translatable("murilloskills.storage_picker.done").formatted(Formatting.GREEN),
                (b) -> close())
    ...
    private void addHeldItem() {
        ...
        toggleItem(Registries.ITEM.getId(held.getItem()).toString());
    }

    private void toggleItem(String id) {
        if (UltmineClientConfig.isStorageWhitelistItem(id)) {
            UltmineClientConfig.removeStorageWhitelistItem(id);
        } else {
            UltmineClientConfig.addStorageWhitelistItem(id);
        }
        UltmineClientConfig.save();
        ClientPlayNetworking.send(new StorageWhitelistSyncC2SPayload(UltmineClientConfig.getStorageWhitelist()));
    }
            ...
            if (col < cols && row < rows) {
                ...
                if (relX - cellInnerX < CELL_SIZE && relY - cellInnerY < CELL_SIZE) {
                    ...
                    if (idx >= 0 && idx < filtered.size()) {
                        ...
                        toggleItem(Registries.ITEM.getId(item).toString());
                        return true;
                    }
                }
            }
    ...
    private void renderHeader(DrawContext context) {
        ...
        context.drawCenteredTextWithShadow(textRenderer,
                Text.translatable("murilloskills.storage_picker.subtitle").copy().formatted(Formatting.GRAY),
    ...
    private void renderGrid(DrawContext context, int mouseX, int mouseY) {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                ...
                boolean inList = UltmineClientConfig.isStorageWhitelistItem(id);
                boolean isHover = mouseX >= cx && mouseX < cx + CELL_SIZE && mouseY >= cy && mouseY < cy + CELL_SIZE;

                int bg = inList ? 0xD0103040 : (isHover ? palette.sectionBgActive() : palette.sectionBg());
                context.fill(cx, cy, cx + CELL_SIZE, cy + CELL_SIZE, bg);

                int borderColor = inList ? palette.accentGold() : (isHover ? palette.accentGold() : palette.sectionBorder());
        ...
        if (hovered != null) {
            ...
            tooltip.add(Text.translatable(UltmineClientConfig.isStorageWhitelistItem(id)
                            ? "murilloskills.storage_picker.click_remove"
                            : "murilloskills.storage_picker.click_add")
                    .copy().formatted(UltmineClientConfig.isStorageWhitelistItem(id) ? Formatting.RED : Formatting.GREEN));
    ...
    private void renderFooter(DrawContext context) {
        int centerX = this.width / 2;
        int listCount = UltmineClientConfig.getStorageWhitelist().size();
        int matchCount = filtered.size();

        String info = Text.translatable("murilloskills.storage_picker.footer",
                String.valueOf(matchCount), String.valueOf(listCount)).getString();
        context.drawCenteredTextWithShadow(textRenderer,
